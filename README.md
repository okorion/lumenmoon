<p align="center">
  <img src="docs/assets/starlight-gate-complete-v1.webp" alt="달빛 아래 네 방향으로 확장된 별빛 관문이 완전히 점등된 루멘문 공동 미션 완성 이미지" width="100%" />
</p>
...
24개의 정규 슬롯이 모두 확정되면 네 방향 대칭 구조가 완성되고, 바닥 문양·좌우 기둥·상단 고리·빛줄기가 차례로 점등됩니다. 완성 상태는 철거할 수 없는 기념물로 보존되며 다음 층의 별빛 관문이 정확히 하나 활성화됩니다.
...
| 상세 패널 | `I` 인벤토리 · `M` 공동 미션 | 상태·미션의 펼치기 버튼 |
| 메뉴 조작 | `Esc`로 Pointer Lock 해제 | 화면 버튼·시트 |

모바일 기본 HUD는 건축 hotbar·블록 수량·조작 버튼만 유지합니다. 좌측 상단 프로필·가방 버튼에서 거점·시설·생산 상태를, 관문 진행 칩에서 미션 상세를 필요할 때만 펼치며 설정은 우측 상단에 분리했습니다. 공동 미션은 기여가 늘 때마다 `5%` 단위로 발광이 성장하고, `25%·50%·75%·100%`에는 구조 연출도 함께 바뀝니다.
